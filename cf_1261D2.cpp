...
struct MathsNTTModAny {
	...
	public:
		...
		vector<ll> convolution_pow(const vector<ll>& a, ll m, int maxcount) {
			int ntt_size = 1;
			while (ntt_size < maxcount) ntt_size *= 2;
			...
			FOR(i, ntt_size) {
				/*ll v = _a[i] % mod;
				_a[i] = 1;
				rep(j, 0, n) (_a[i] *= v) %= mod;*/
				_a[i] = mod_pow(_a[i], m, mod);
			}
	...
	vector<ll> solve(vector<ll> a, vector<ll> b, int mod = MODMOD) {
		for (auto& x : a) x %= mod;
		for (auto& x : b) x %= mod;
	...
	vector<int> solve(vector<int> a, vector<int> b, int mod = MODMOD) {
		vector<ll> x(all(a));
		vector<ll> y(all(b));
	...
	vector<mint> solve(vector<mint> a, vector<mint> b, int mod = MODMOD) {
		int n = a.size();
		vector<ll> x(n);
	...
	vector<ll> solve_pow(vector<ll> a, ll m, int maxcount, int mod = MODMOD) {
		for (auto& x : a) x %= mod;

		NTT_1 ntt1; NTT_2 ntt2; NTT_3 ntt3;
		assert(ntt1.get_mod() < ntt2.get_mod() && ntt2.get_mod() < ntt3.get_mod());
		auto x = ntt1.convolution_pow(a, m, maxcount);
		auto y = ntt2.convolution_pow(a, m, maxcount);
		auto z = ntt3.convolution_pow(a, m, maxcount);
	...
	vector<mint> solve_pow(vector<mint> a, ll m, int maxcount, int mod = MODMOD) {
		int n = a.size();
		vector<ll> x(n);
		rep(i, 0, n) x[i] = a[i].get();

		auto z = solve_pow(x, m, maxcount, mod);
		vector<mint> res;
		fore(aa, z) res.push_back(aa);
...
struct FormalPowerSeries {
	...
	Poly mul_1_plus_x_n(Poly as, int n) {
		...
		return div_1_minus_x(p1);
	}

	// (a0+a1x+a2x^2+...)^n = b0+b1x+...+b[maxcount-1]x^(maxcount-1)
	Poly fastpow(Poly a, ll n, int maxcount) {
		MathsNTTModAny ntt;
		return ntt.solve_pow(a, n, maxcount);
	}

	// (a0+a1x+a2x^2+...) / x^n
	Poly shiftLeft(Poly a, int n) {
		int sz = a.size();
		rep(i, 0, sz) {
			if (i + n < sz) a[i] = a[i + n];
			else a[i] = 0;
...
void _main() {
	...
	FormalPowerSeries<mint> FPS;
	int cnt = 0;
	rep(i, 0, N) if (H[i] == H[(i + 1) % N]) cnt++;
	vector<mint> f = { 1, K - 2, 1 };
	N -= cnt;
	auto fp = FPS.fastpow(f, N, 1<<17);
	fp = FPS.shiftLeft(fp, N);
...
h[i]==h[i+1]であればi番目は何であれ余り関係ないので、その個数をcntとすると、k^cnt通りはまずある。
h[i]!=h[i+1]の場合がN-cnt個残る。
それぞれは独立で考えることができ、場面も統一できるので、
1通りで-1, 1通りで+1, K-2通りで0となる選択肢がN-cnt個あることになる。
これは-1がi個, +1がj個で全探索すると、0<j-iのときのcomb(N, i)*comb(N-i, j)*(K-2)^(N-i-j)*k^cntの総和が答え。
これがEasy解法。

これを後は高速化していく。
普通に変形してFFTでもいいんだけど、FPSで高速化することを考える。
k^cnt分は最後にかけるとして、comb(N, i)*comb(N-i, j)*(K-2)^(N-i-j)の総和を求める。
n -= cntで以降は考えよう。
